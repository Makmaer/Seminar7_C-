...
int SumEllementsArray(int[,] array)
{      int sum =0;
...
int[,] PoiskChisla2D(int[,] array, int chislo)
{
    int[,] newArray = new int[10,2]; newArray[0,0] = -1; newArray[0,1] = -1; int str = 0;
    for (int i = 0; i < array.GetLength(0); i++)
        for (int j = 0; j < array.GetLength(1); j++)          
...
void PrintMasIntIndex (int[,] array)
...
double[] SredneArifm( int[] array, int rows)
{
...
int n = Convert.ToInt32(Console.ReadLine());
int[,] myArray = RandomD2Array(m, n, 1, 9);
int[] rez = SummaPoStolbcam(myArray);
ShowD2Array(myArray);
//PrintMasDouble(rez);
